...
- Makes **`/usr/local`** owned by your user, replaces **`/usr/local/bin/tar`** with a **symlink** to Chrome OS’s **`/usr/bin/tar`** or **`/bin/tar`**, and puts **`/usr/bin` and `/bin` first in `PATH`**.
